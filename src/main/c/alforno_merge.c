...
static PastaValue *do_aggregate(AlfContext *ctx, AlfResult *result) {
    ...
    if (!out) {
        alf_set_error(result, ALF_ERR_ALLOC, ALF_PASS_MERGE, NULL, "allocation failed");
        return NULL;
    }
    ...
    for (size_t i = 0; i < ctx->input_count; i++) {
        ...
        for (size_t j = 0; j < pasta_count(inp); j++) {
            ...
            if (existing) {
                ...
                if (!merged) {
                    alf_set_error(result, ALF_ERR_ALLOC, ALF_PASS_MERGE, key, "allocation failed");
                    pasta_free(out);
                    return NULL;
                }
...
static PastaValue *build_conflated_section(const PastaValue *merged,
    ...
    PastaValue *out = pasta_new_map();
    if (!out) {
        alf_set_error(result, ALF_ERR_ALLOC, ALF_PASS_MERGE, secname, "allocation failed");
        return NULL;
    }
...
static PastaValue *do_conflate(AlfContext *ctx, AlfResult *result) {
    if (!ctx->recipe || pasta_type(ctx->recipe) != PASTA_MAP) {
        alf_set_error(result, ALF_ERR_BAD_RECIPE, ALF_PASS_MERGE, NULL,
    ...
    for (size_t ri = 0; ri < pasta_count(ctx->recipe); ri++) {
        ...
        if (pasta_type(rule) != PASTA_MAP) {
            alf_set_error(result, ALF_ERR_BAD_RECIPE, ALF_PASS_MERGE, secname,
        ...
        if (!consumes || pasta_type(consumes) != PASTA_ARRAY
            ...
            alf_set_error(result, ALF_ERR_MISSING_CONSUMES, ALF_PASS_MERGE, secname,
        ...
        if (merge_val) {
            ...
            if (ms && strcmp(ms, "collect") == 0)
                ...
                alf_set_error(result, ALF_ERR_BAD_RECIPE, ALF_PASS_MERGE, secname, msg);
                pasta_free(out);
                return NULL;
            }
        }
